			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("Person") === oObject.Person;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].Person;
										} else {
											sCurrentId = aItemsNotInTheList[0].Person;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("EtPersonSet", {
											Person : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("Person"),
				...
				assertions : {
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all EtPersonSet to be able to check for the total amount
